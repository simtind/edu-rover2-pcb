...
#define BATTERY_ADC_TO_VOLT(adc_value)     (int16_t)map(adc_value,  0, 1023, 0, 150 * SENSOR_SCALE)  // 0V to 5V in, multiply by 3 for divider.
#define PRESSURE_ADC_TO_KPA(adc_value)     (int16_t)map(adc_value, 41, 1023, 0, 250 * SENSOR_SCALE) // 0.2V to 5V -> 0kPa to 250kPa
#define TEMPERATURE_ADC_TO_DEGC(adc_value) (int16_t)map(adc_value,  0, 1023, 0, 50  * SENSOR_SCALE)  // 5V / (10mV / degC) = 50 degC  
...
#define MOTOR_ADC_TO_mA(adc_value)         (int16_t)map(adc_value, 0, 1023, -5000, 5000)
